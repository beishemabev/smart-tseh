  ...
  if (taskTypes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 sm:p-12 text-center bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[2rem] border border-white/40 dark:border-slate-800 shadow-[0_8px_32px_-8px_rgba(0,0,0,0.1)] transition-colors">
        ...
        <h3 className="text-base font-semibold text-slate-800 dark:text-white mb-1">Нет операций</h3>
  ...
  return (
    <div className="space-y-3">
      {taskTypes.map((task) => (
        <div 
          key={task.id} 
          className="relative flex items-center p-4 sm:p-5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-[1.5rem] border border-white/40 dark:border-slate-800 shadow-sm transition-all hover:shadow-md hover:border-white/80 dark:hover:border-slate-700 group"
        >
          <div className="flex-1 min-w-0 pr-4">
            <h4 className="text-[17px] font-bold text-slate-900 dark:text-white truncate">
              {task.name}
            </h4>
            <div className="mt-1.5 text-[15px] font-bold text-emerald-600 dark:text-emerald-500">
          ...
          <div className="flex-shrink-0 flex items-center">
            <div className="hidden sm:flex items-center space-x-2">
              <Button variant="ghost" size="icon" onClick={() => onEdit(task)} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800">
                <Edit2 className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => onDelete(task)} className="text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10">
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-48 rounded-xl bg-white dark:bg-slate-900 shadow-lg ring-1 ring-black/5 dark:ring-white/10 z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                    <button
                      ...
                      className="flex items-center w-full px-4 py-3 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 border-b border-gray-100 dark:border-slate-800"
                    >
